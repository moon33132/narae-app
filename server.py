...
def student_dashboard():
    ...
    return render_template("student.html", num=num, notices=notices,
...
def word_test_submit(wid):
    ...
    if str(num) in all_ans: return jsonify({"ok":False,"msg":"이미 제출했어요!"})
    data    = request.get_json()
    student = data.get("answers", {})  # {"0": "apple", "1": "바나나", ...}
    direction = test.get("direction", "ko_to_en")  # ko_to_en / en_to_ko / mixed
    score, total, results = 0, len(test["words"]), []
    for i, word in enumerate(test["words"]):
        k = str(i)
        s_ans = student.get(k, "").strip()
...
def quiz_add():
    ...
    qraw  = request.form.get("questions","").strip()
    if title and qraw:
        qs = []
        for line in qraw.split("\n"):
            ...
            if "|" in line:
                t,cr = line.split("|",1)
                qs.append({"text":t.strip(),"type":"choice","choices":[c.strip() for c in cr.split(",") if c.strip()]})
            else:
...
def quiz_results(qid):
    ...
    return render_template("quiz_results.html", quiz=quiz,
        answers=load_json(quiz_answers_path(qid),{}), total=STUDENT_COUNT)
...
def exam_add():
    ...
    ak_raw  = request.form.get("answer_key","").strip()
    answer_key = {}
    for item in ak_raw.replace("\n",",").split(","):
        item=item.strip()
        if ":" in item:
            qn,ans=item.split(":",1); answer_key[qn.strip()]=ans.strip()
    if title:
